...
resource "aws_subnet" "misc" {
  ############################
  for_each = local.misc_subnets

  vpc_id = aws_vpc.this[0].id

  cidr_block = lookup(
    var.subnets.misc, "cidr_blocks", null
  ) != null ? element(var.subnets.misc.cidr_blocks, each.value) : cidrsubnet(var.vpc.cidr_block, local.plus_bits, each.value + length(local.public_subnets))
  ...
  map_public_ip_on_launch = lookup(
    var.subnets.misc, "map_public_ip_on_launch", null
    ) != null ? var.subnets.misc.map_public_ip_on_launch : lookup(
  ...
  assign_ipv6_address_on_creation = lookup(
    var.subnets.misc, "assign_ipv6_address_on_creation", null
    ) != null ? var.subnets.misc.assign_ipv6_address_on_creation : lookup(
  ...
  ipv6_cidr_block = lookup(
    var.subnets.misc, "ipv6_cidr_block", null
  ) != null ? element(var.subnets.misc.ipv6_cidr_block, each.value) : null

  # customer_owned_ip
  map_customer_owned_ip_on_launch = lookup(
    var.subnets.misc, "map_customer_owned_ip_on_launch", null
    ) != null ? var.subnets.misc.map_customer_owned_ip_on_launch : lookup(
  ...
  customer_owned_ipv4_pool = lookup(
    var.subnets.misc, "customer_owned_ipv4_pool", null
  ) != null ? element(var.subnets.misc.customer_owned_ipv4_pool, each.value) : null

  outpost_arn = lookup(
    var.subnets.misc, "outpost_arn", null
    ) != null ? var.subnets.misc.outpost_arn : lookup(
